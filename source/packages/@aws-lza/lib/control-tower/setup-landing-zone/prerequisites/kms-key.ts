...
export abstract class KmsKey {
   ...
   */
  private static async aliasExists(client: KMSClient, keyAlias: string): Promise<boolean> {
    const aliasItems = await KmsKey.getAliases(client);
    const aliasNames = aliasItems.map(aliasItem => aliasItem.AliasName);
    return aliasNames.includes(keyAlias);
  }
